...
if [ $(ls /mnt/gentoo 2>>/dev/null) ]; then
	...
	config_file_path=/mnt/gentoo/opt/lbs/conf/vm_profile
else
	mkdir -p /opt/lbs/conf
	config_file_path=/opt/lbs/conf/vm_profile
fi
...
#set  vm_type
	...
	vm_type="COMPUTE"
	echo "Please input vm_type:(CONTROL | CONTROL_HA | COMPUTE | STORAGE)"
	read -p "(Default vm_type: COMPUTE):" vm_type
	if [ "$vm_type" = "" ]; then
		vm_type="COMPUTE"
	fi
	echo "==========================="
	...
	echo "==========================="
	echo vm_type="$vm_type" > $config_file_path
...
#set vm_service_ip
	...
	vm_service_ip="10.1.1.11"
	echo "Please input the vm_service_ip :(like xxx.xxx.xxx.xxx)"
	read -p "(Default is 10.1.1.11 ):" admin_vlan
	if [ "$vm_service_ip" = "" ]; then
		vm_service_ip="10.1.1.11"
	fi
	echo "==========================="
	echo vm_service_ip="$vm_service_ip"
	echo "==========================="
	echo vm_service_ip="$vm_service_ip" >> $config_file_path
...
echo "====================================================================="
echo "=========================== Check install ==========================="
if [ $(ls /mnt/gentoo/opt/lbs/conf/vm_profile 2>>/dev/null) ] || [ $(ls /opt/lbs/conf/vm_profile 2>>/dev/null) ]; then
